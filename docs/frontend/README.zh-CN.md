...
API 契约见 [API 契约](../api/README.zh-CN.md)。稳定架构基线见 [PROJECT.md](../../PROJECT.md)。
...
- Guest composer 固定 `workspaceWrite` access，只有 `gpt-5.6-sol`、`gpt-5.6-terra`、`gpt-5.6-luna`、`gpt-5.5` 以及 low/medium effort 可用；没有附件和 Web Search。真实 Guest skill 列表只返回 name、display name、description、scope 与短期 handle，不返回技能文件路径。
